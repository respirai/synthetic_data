...
**Upstream codebase:**  
- https://github.com/DreamStudioAI/sim_gan/tree/master

**This repository:**  
- https://github.com/respirai/synthetic_data/tree/main
...
### 5.1 Data & Domain Adaptation

- **COPD-specific data ingestion:**
  Added VivaLNK/COPD dataset loaders (CSV-based), most notably:
  - `data_reader/ecg_dataset_copd_from_csv.py`

- **Sampling-rate alignment:**
  Introduced resampling utilities to bridge VivaLNK **128 Hz** ECG with SimGAN’s internal **360 Hz** pipeline:
  - `data_reader/transforms_resample.py`
  
  The internal SimGAN architecture remains unchanged (360 Hz); resampling is applied only at input/output boundaries.
...
### 5.3 Preparation for Window-Level ECG Generation

- **Multi-beat / window-oriented simulator step:**
  Extended the Euler-based simulator logic toward window-level operation (beyond single-beat synthesis):
  - `dynamical_model/Euler/euler_window.py`

  This prepares the pipeline for generating **consecutive beats / ECG windows** with same-patient morphology consistency.

---


## 7) Expected Artifacts / Outputs
